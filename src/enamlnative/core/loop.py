...
"""
import enamlnative
from atom.api import Atom, Value, Subclass, Callable, Str
        ...
        with enamlnative.imports():
            for impl in [
                ...
                if impl.available():
                    ...
                    return impl()
        raise RuntimeError(
            "No event loop implementation is available. Install tornado."
        )
    ...
    def _default_future(self):
        ...
        class Future(BaseFuture):
            ...
            def then(self, callback):
                ...
                """
                self.add_done_callback(partial(self.safe_callback, callback, False))
                return self
...
class BuiltinEventLoop(TornadoEventLoop):
    ...
    def available(cls):
        try:
            ...
            print(
                "Error: Failed to load the builtin event loop. "
