...
class GridLayout(ViewGroup):
    ...
    #: ColumnCount is used only to generate default column/column indices when they are not
    #: specified by a component's layout parameters.
    columns = d_(Int(1))

    #: When this property is true, GridLayout is forced to place the column boundaries so that their
    #: associated grid indices are in ascending order in the view.
    column_order_preserved = d_(Bool())

    #: RowCount is used only to generate default row/column indices when they are not specified
    #: by a component's layout parameters.
    rows = d_(Int(1))

    #: When this property is true, GridLayout is forced to place the row boundaries so that their
    #: associated grid indices are in ascending order in the view.
