...
def main():
    ...
    app = AndroidApplication(
        debug=True, dev="remote", load_view=load_view  # "10.0.2.2" # or 'server'
    )
...
def run_gestures(app):
    for i in range(30):
        ...
        t = i * 2000
        app.timed_call(
            t, sh.adb, *"shell input swipe 250 300 -800 300".split(), _bg=True
        )
        #: Tap a few places
        for j in range(4):
            app.timed_call(
                t + i * 200, sh.adb, *"shell input tap 500 150".split(), _bg=True
            )
