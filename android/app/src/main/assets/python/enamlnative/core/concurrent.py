...
from atom.api import Atom, Instance, Bool, List
...
class _TracebackLogger(Atom):
    ...
    """
    formatted_tb = Instance(object)
    exc_info = Instance(object)
...
class Future(Atom):
    ...
    """
    _done = Bool()
    _result = Instance(object)
    _exc_info = Instance(object)
    _log_traceback = Bool()
    _tb_logger = Instance(object)
    _callbacks = List()
